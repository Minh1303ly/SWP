...
public class SlidersDAO extends DBContext {
    ...
    public Slider getSliderById(int id) {
        Slider slider = null;
        String sql = "SELECT [Id], [title], [img], [backLink], [slider_status], user_id FROM sliders WHERE id = ?";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    ...
                    slider.setUser(udao.getUserById(rs.getInt("user_id")));
                }
            }
        } catch (SQLException ex) {
    ...
    public List<Slider> getSliders(String status, String search, int page, int pageSize) {
        ...
        String sql = "SELECT [Id], [title], [img], [backLink], [slider_status], user_id FROM [dbo].[sliders] WHERE 1=1";
